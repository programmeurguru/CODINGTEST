...
namespace EventBus2
{
	...
	void EventBus::notify()
	{
		...
		while (!_stop)
		{
			...
			std::vector<Subscriber *> subscribers = _subscribersMap.getSubscribersFromEvent(anEvent);
			for (auto it = subscribers.begin(), itEnd = subscribers.end(); it != itEnd; ++it)
			{
				(* it)->consume(anEvent);
			}

			std::vector<Subscriber*> subscriberPreds = _subscribersPredicateMap.getSubscribersFromEvent(anEvent);
