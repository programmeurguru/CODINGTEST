...
namespace hsbctest4
{
	TEST(TestStatistics, MeanCalculatorTest)
	{
		hsbctest4::MeanCalculator m;
		m.AddAndcalculate(1);
		EXPECT_EQ(m.get(), 1.0);
		m.AddAndcalculate(1);
		EXPECT_EQ(m.get(), 1.0);
	...
	TEST(TestStatistics, ModCalculatorTest)
	{
		hsbctest4::ModCalculator m;
		m.AddAndcalculate(1);
		EXPECT_EQ(m.get(), 1);
		m.AddAndcalculate(1);
		EXPECT_EQ(m.get(), 1);
	...
	TEST(TestStatistics, PctileCalculatorTest)
	{
		hsbctest4::PctileCalculator m;
		m.AddAndcalculate(1);
		EXPECT_EQ(m.get(0), 1);
	...
	TEST(TestStatistics, StatisticsProviderTest)
	{
	...
	TEST(TestStatistics, EventStatisticsTest)
	{
	...
	TEST(EventBusTest4, ExtractLastest)
	{
		...
		StampedEvent<EventStatistics> e1(Event<EventStatistics>("EV1", 10.0, evs1));
		qe.push(e1);
		eventMap[e1.getEvent().getEventType()] = e1.getcount();

		StampedEvent<EventStatistics> e2(Event<EventStatistics>("EV2", 11.0, evs1));
		qe.push(e2);
		eventMap[e2.getEvent().getEventType()] = e2.getcount();

		StampedEvent<EventStatistics> e3(Event<EventStatistics>("EV1", 12.0, evs2));
		qe.push(e3);
		eventMap[e3.getEvent().getEventType()] = e3.getcount();

		StampedEvent<EventStatistics> e4(Event<EventStatistics>("EV3", 13.0, evs1));
		qe.push(e4);
		eventMap[e4.getEvent().getEventType()] = e4.getcount();

		StampedEvent<EventStatistics> e5(Event<EventStatistics>("EV2", 14.0, evs3));
		qe.push(e5);
		eventMap[e5.getEvent().getEventType()] = e5.getcount();

		StampedEvent<EventStatistics> e6(Event<EventStatistics>("EV3", 15.0, evs4));
		qe.push(e6);
	...
	TEST(EventBusTest4, conflationTest)
	{
		std::vector<std::thread> consumers;
		EventQueue evQueue;
		SafeSubscribersPredicateMap<EventStatistics> subscribersPredicateMap;
		hsbctest4::EventBus EventBus(subscribersPredicateMap, evQueue, consumers);
